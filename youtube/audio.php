...
return FALSE;
  ...
  #etc i,#youtube i {
    ...
    '.$g.'" controls title="'.$video_title.'"></audio><br><p><a id="link-none" href="https://aic-group.sytes.net'.$g.'" download="'.$title.' - '.$k.'.wav">'.$video_title.'をダウンロードする ('.ByteToUnit($filesize).')</a><br>(ファイルは一時間以内に消去されます。)</p>';
    echo '    </div></div><h2 class="h2 title">別の音声をダウンロード</h2>
    <div class="box" style="text-align:center;">
    <form action="/youtube/a" style="text-align: center; margin-bottom: 0;" method="post">
    <input type="text" class="download" name="audio" required>
        <input type="submit" class="download" id="audio" value="ダウンロード" required>
    </form>
    </div>';
